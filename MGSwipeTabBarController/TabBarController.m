...
- (UIScrollView*) scrollView {
    if (!_scrollView) {        
        ...
        NSLog(@"self.viewControllers count : %d",self.viewControllers.count);
    }
    return _scrollView;
}
...
- (void) setSelectedIndex:(NSUInteger)selectedIndex
{
    ...
    NSLog(@" frame.origin.x %f = frame.size.width %f * selectedIndex %d",frame.origin.x , frame.size.width , selectedIndex);
...
- (void) viewDidLoad
{
    ...
    if (self.tabBar){
       ...
       // _tabBar.frame=CGRectMake(0, 50, self.view.frame.size.width, self.view.frame.size.height);
    }
